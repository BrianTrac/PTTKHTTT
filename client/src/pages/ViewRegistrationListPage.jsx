		...
		const loadData = async () => {
			...
			const mappedData = rawData.map((item, index) => {
				...
				return {
					...
					registrationDate: (item.NgayDangKy || "")
						.replace("T", " ")
						.slice(0, 19),
		...
		>
			<div className="max-w-7xl mx-auto bg-white rounded-lg border border-gray-300 shadow-lg p-8">
				...
				<div className="text-center mb-8">
					<h1 className="text-3xl font-bold text-[#033060]">
				...
				<div className="mb-8 p-5 border border-gray-300 rounded-md bg-gray-100">
					<div className="flex items-center gap-4 flex-wrap">
						<label
							htmlFor="searchId"
							className="text-[#033060] font-medium whitespace-nowrap"
						>
							Tìm kiếm bằng ID:
						</label>
						<input
						...
						<thead className="bg-[#033060] text-white">
							<tr>
								{[
									...
									<th
										key={header}
										className="p-3 font-semibold border-b border-[#c7d6ea] text-center"
									>
										{header}
									</th>
								))}
						...
						<tbody>
							{paginatedData.map((item, index) => (
								...
								>
									...
									<td className="p-3 text-center border-b border-[#e0e7ef]">
										<span
											className={`${item.status.color} px-3 py-1 rounded-full text-sm`}
										>
											{item.status.label}
										</span>
									</td>
